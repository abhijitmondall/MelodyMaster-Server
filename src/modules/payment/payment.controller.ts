...
const verifyCheckoutSession = catchAsync(async (req: Request, res: Response) => {
  ...
  if (!session_id) {
    res.status(400).json({ success: false, message: "session_id query param is required." });
    return;
  }
...
const handleWebhook = catchAsync(async (req: Request, res: Response) => {
  ...
  await paymentService.handleWebhookEvent(req.body as string | Uint8Array, signature);
